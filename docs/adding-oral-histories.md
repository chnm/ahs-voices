...
This guide is written for the people who manage the archive. It covers everything from gathering your materials to seeing the finished interview appear on the site.
...
- [ ] **The transcript** — a typed version of what was said (a Word doc or plain text is fine).
...
| **Date Recorded** | When the interview was recorded. Use a clear format like `2003-05-14` (year-month-day) or `January 5, 2024` (full month-day-year). |

### Recommended extras (optional, but worth adding)

| Field | What to put here |
|---|---|
| **Abstract** | A short one- or two-sentence "teaser." This is what shows up in browse listings and search results, so a punchy summary here helps people decide to click. If you leave it blank, the longer Summary is used instead. |
...
1. Scroll to the **Media** section (a tab or section on the item form).
2. Click to add media, choose **Upload**, and select your audio or video file.
3. To add a photo, upload it as another media item the same way.

A few notes:

- **Audio and video both work.** `.mp3`, `.m4a`, and `.mp4` files all play in the built-in player on the interview page.
- **The photo becomes the thumbnail automatically**: you don't need to crop or resize it. The site generates the small preview images for you.
...
1. **Assign it to a collection.** Look for the **Item sets** section and add this interview to the relevant collection (e.g. *Project Shirlington*). Collections are how interviews are grouped on the "Explore by theme" part of the site.

2. **Assign it to the site.** Make sure the item is added to the **Arlington Stories** site (there's a "Sites" section on the form). An interview that isn't assigned to the site won't appear in the public listings, even if everything else is filled in.
...
The site has a **Map** page showing where interviews are connected to places around Arlington. To add an interview to the map:

1. Open the item in admin (**Items → click the interview → Edit**).
2. Find the **Mapping** section on the form.
3. Drop a marker at the relevant location.
4. Save.
...
- **Be consistent with Topics and Series Titles.** Using the same wording each time keeps the browse-by-theme filters tidy.
...
- [ ] **Abstract** written (the teaser for listings)
- [ ] **Topics** added
